/* WEB3-BUILDERS-ALLIANCE CLUSTER1 CODE JOURNAL
PLEASE FIND ALL THE COMMENTS RELATED TO THE JOURNAL WITHIN THESE BLOCK COMMENTS: /* */
*/
...
/*
crates used in contract:
- error - ContractError (enum type with 7 items )
- messages necessary for contract handling (either enums or structs)
- state constants + Config struct (used for instantiation and further reference via CONFIG.LOAD)
*/
...
/*
An instantiate method that:
- sets an ADMIN constant with an msg.admin address (which is an Option<String> type)
- instantiates min_bond: Uint128 variable that has to be set to a value of at least 1 to ensure that there is max() method from a standard library used
- instantiates config: Config variable that takes 4 params: 
    - denom of the token
    - tokens_per_weight (not sure what this variable does)
    - previously set min_bond variable
    - unbonding_period: Duration - a period that needs to pass for the token to be released after staking
- saves config var's reference to the storage
- saves a total stake of all users as 0 (which has to be a reference: &0 due to .save method definition)
*/
...
/*
execute function implementing 7 ExecuteMsg items:
- UpdateAdmin that attempts to update ADMIN constant with new_admin param
- AddHook registers new hook address under HOOKS constant
- RemoveHook removes registered hook from HOOKS
- Bond updates sender's (info.sender) STAKE with provided amount (Balance::from(info.funds)), 
saves it to new_stake var and attempts to run update_membership() that performs following steps:
    - recalculates new membership weight (according to the new_stake resulting from previous step)
    - updates MEMBERS const with new weight
    - updates TOTAL storage with new weight
    - prepares hooks (using HOOKS const)
- Unbond does the same as Bond but reduces sender's take instead of increasing and creates a new claim (CLAIMS.create_claim)
for a sender with given amount of tokens and expiration time
- Claim provides a mechanism to claim unbonded tokens:
    - retrieves the amount of tokens to be claimed from CLAIMS storage
    - sends tokens back to the sender
- Receive - not sure what this item is responsible for as there's a message of type Cw20ReceiveMsg
*/
